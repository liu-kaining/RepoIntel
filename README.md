...
Dry-run 会生成 17 个模拟候选仓库，走完完整三层漏斗，并在 `hugo-site/content/posts/` 与 `output/wechat_digest.txt` 产出示例内容。粗筛后的 15 个项目会写入 `output/processed_repos.json`（本地模式），其中最多 10 个达到发布阈值后生成 Hugo 文章。

### 正式运行

推荐先按 `.env.example` 准备环境变量：
...
`hugo-site/hugo.toml` 默认使用相对 URL：
...
这可以直接适配 GitHub Pages 项目站点。若你有自定义域名，也可以把 `baseURL` 改成对应域名。
